# ui/delivery_challan_window.py
from PyQt5.QtWidgets import (
    QWidget, QLabel, QLineEdit, QTextEdit, QPushButton, QHBoxLayout, QVBoxLayout,
    QGridLayout, QTableWidget, QTableWidgetItem, QComboBox, QMessageBox, QHeaderView,
    QSpinBox, QDoubleSpinBox, QCompleter
)
from PyQt5.QtCore import Qt, QDateTime
from PyQt5.QtGui import QIntValidator

from models import delivery_model
# Try to import stock helpers; fallback to sqlite direct access
try:
...
def fetch_company_profile():
    """Return single company_profile row as dict or None."""
...
def fetch_stock_list():
    """Return list of stock dicts: [{'code','name','hsn_code','unit'}...]"""
    # Try to use stock_model if available
    try:
...
class DeliveryChallanWindow(QWidget):
    def __init__(self, parent=None):
        ...
        self.setWindowTitle("Create Delivery Challan")
    ...
    def setup_ui(self):
        ...
        # Header grid: challan no, date/time, company and to-address block
        ...
        company_label.setAlignment(Qt.AlignTop)
        ...
        # Add completer for description suggestions
        ...
        self.save_btn = QPushButton("Save Challan")
    ...
    def add_row(self, prefill=None):
        """
        Add a new editable row. prefill: dict with keys item_code,item_name,hsn,qty,unit
        """
        row_pos = self.table.rowCount()
        self.table.insertRow(row_pos)

        # Item Code: editable combo populated with stock codes, allow manual typing
        ...
        # Allow manual editing of name/hsn/unit by double-click (default)
        self.update_total_qty()
    ...
    def on_code_changed(self, row, code_text):
        """
        When user selects or types an item code, auto-fill name/hsn/unit if exists in stock.
        """
        ...
        # else: leave manual values as-is
    ...
    def table_to_items(self):
        """
        Convert current table rows to list of item dicts for create_challan().
        """
        items = []
        for r in range(self.table.rowCount()):
    ...
    def validate_header(self):
        # At minimum we require a To address and at least one item
    ...
    def save_challan(self):
        ...
        try:
            challan_id, challan_no = delivery_model.create_challan(
                header, items)
        ...
        # update description suggestions completer with new entry
        suggestions = delivery_model.get_description_suggestions("", limit=50)
        self.desc_completer.model().setStringList(suggestions)

        QMessageBox.information(self, "Saved", f"Challan saved: {challan_no}")
